...
<button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
         <span class="sr-only">Toggle Navigation</spa n>
 ...
 <br><br><br><br><br>
  ...
  if(isset($_POST['submit'])){
    ...
    if (!empty($reg_no) && !empty($password)) {
      $query="SELECT reg_no FROM students WHERE reg_no ='$reg_no' && password=md5('$password')";
  ...
  mysql_close($conn)
       ?>
       <form action="student_login.php" method="POST" role="form" class="form-horizontal">
  <div class="form-group">
    <label class="control-label col-sm-2" for="reg_no">RegNumber:</label>
    <div class="col-sm-5">
      <input type="text" class="form-control" name="reg_no" placeholder="Enter RegNumber">
    </div>
  </div>
  <div class="form-group">
